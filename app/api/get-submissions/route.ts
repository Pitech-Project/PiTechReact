  ...
  try {
    ...
    if (!type || (type !== "career" && type !== "contact")) {
      return new NextResponse(
        'Missing or invalid "type" parameter. Must be "career" or "contact".',
        {
          status: 400,
        },
      );
    ...
    const collectionName =
      type === "career" ? "careerSubmissions" : "submissions";
    const collection = db.collection(collectionName);
    submissions = await collection.find({}).toArray();

    if (submissions.length === 0) {
      const filePath =
        type === "career" ? careerSubmissionsFilePath : submissionsFilePath;
      try {
