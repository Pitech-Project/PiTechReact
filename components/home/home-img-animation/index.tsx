...
const HomeImgAnimation: React.FC = () => {
  const canvasParentRef3 = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    const sketch: Sketch = (p: p5Types) => {
      ...
      p.preload = () => {
        img = p.loadImage(
          "https://i.postimg.cc/hjhTdBSn/pawel-czerwinski-V5vtl-Id8s-Kw-unsplash.jpg",
        );
      };

      p.setup = () => {
        const container = canvasParentRef3.current;
        if (!container) return;

        canvasW = container.offsetWidth;
      canvasW = container.offsetWidth;
      canvasH = container.offsetHeight || window.innerHeight;
...
p.touchMoved = () => {
  ...
  return (
    <CanvasContainer ref={canvasParentRef3}>
