...
import { DirectionLink, OurTeamGrid, OuterSection, OwlCarouselBox, SmallFullStop } from "@/app/styles/MUI/common.styled";
            ...
            <OuterSection sx={{ width: '90%', gap: 0 }}>
                ...
                >
                    <Stack className="SubTitlemarginBottom">
                        <Typography color='custom.black' variant='body_4' letterSpacing={'2.4px'} marginBottom={2} fontWeight={600} display={'block'}>OUR OFFICES <SmallFullStop /></Typography>
                        <Typography color='custom.black' variant='body_6' display={'block'}>Offices at different locations</Typography>
                    </Stack>
                </motion.div>
                <motion.div
                    initial={{ opacity: 0, x: -100 }}
                    ...
                    <Grid container spacing={2}>
                        {ouroffices.map((ouroffices) => (
                            <OurTeamGrid size={{ xs: 12, md: 4 }} key={ouroffices.id}>
                                <Image src={ouroffices.img} alt='out office' style={{ width: '-webkit-fill-available', height: 'auto' }} />
                                <OwlCarouselBox marginTop={"-8px"}>
                                    <Box>
                                        <Typography variant='body_5' display={'block'} color='custom.white2' marginBottom={0.2}>{ouroffices.title}</Typography>
                                        <Typography variant='body_4' fontWeight={400} color='custom.white3'>{ouroffices.location}</Typography>
                                    </Box>
                                    <Stack flexDirection={'row'} alignItems={'center'} gap={1}>
                                        <Typography variant="small_typography" color="custom.white2">GET DIRECTIONS</Typography>
                                        <DirectionLink target="blank" href={ouroffices.link} className=''>
